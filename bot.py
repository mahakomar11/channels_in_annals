...
def get_message(message):
    ...
    if message.text == "Инструкция":
        ...
        bot.send_message(
            user_id, display_channels(user_id), reply_markup=inline_keyboard
        )
        ...
        bot.send_message(
            user_id, search_channels(user_id, message.text, max=1)
        )
...
def add_channel_by_link(user_id, message):
    ...
    if "t.me/" not in link:
        return (
            'Не вижу ссылки. Должна быть либо ссылка, либо "Название ссылка",'
            " через пробел."
        )
    if link != message:
        ...
        try:
            ...
            return (
                "Не могу достать название, вероятно канал засекречен. Нажми"
                ' "Добавить канал вручную" и пришли "Название ссылка", через'
                " пробел."
            )
...
def search_channels(user_id, text, max=None):
    ...
    return _create_message_from_channels(
        found_channels,
...
def _create_message_from_channels(channels, no_channels_message=None):
    if no_channels_message is None:
        no_channels_message = (
            "Каналы не добавлены! Чтобы добавить канал, перешли мне сообщение"
            " из него."
        )
    if len(channels) == 0:
